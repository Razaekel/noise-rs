...
/// roughness.
#[derive(Clone, Debug)]
pub struct Turbulence<F, Source> {
...
macro_rules! impl_turbulence {
    ($f:ty) => {
        impl<Source> Turbulence<$f, Source> {
            pub const DEFAULT_SEED: u32 = 0;
            pub const DEFAULT_FREQUENCY: $f = 1.0;
            pub const DEFAULT_POWER: $f = 1.0;
            ...
            pub fn set_frequency(self, frequency: $f) -> Self {
                Self {
                    frequency,
            ...
            pub fn set_power(self, power: $f) -> Self {
                Self { power, ..self }
            }
...
impl<F, Source> Seedable for Turbulence<F, Source> {
...
macro_rules! turbulence_2d {
    ($f:ty) => {
        impl<Source> NoiseFn<$f, 2> for Turbulence<$f, Source>
        where
            Source: NoiseFn<$f, 2>,
        {
...
macro_rules! turbulence_3d {
    ($f:ty) => {
        impl<Source> NoiseFn<$f, 3> for Turbulence<$f, Source>
        where
            Source: NoiseFn<$f, 3>,
        {
...
macro_rules! turbulence_4d {
    ($f:ty) => {
        impl<Source> NoiseFn<$f, 4> for Turbulence<$f, Source>
        where
            Source: NoiseFn<$f, 4>,
        {
            fn get(&self, point: [$f; 4]) -> $f {
                // First, create offsets based on the input values to keep the sampled
                // points from being near a integer boundary. This is a result of
                // using perlin noise, which returns zero at integer boundaries.
                let x0 = point[0] + 12414.0 / 65536.0;
                ...
                let x_distort =
                    point[0] + (self.x_distort_function.get([x0, y0, z0, u0]) * self.power);
                let y_distort =
                    point[1] + (self.y_distort_function.get([x1, y1, z1, u1]) * self.power);
                let z_distort =
                    point[2] + (self.z_distort_function.get([x2, y2, z2, u2]) * self.power);
                let u_distort =
                    point[3] + (self.u_distort_function.get([x3, y3, z3, u3]) * self.power);
