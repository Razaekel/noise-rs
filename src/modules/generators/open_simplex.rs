...
impl<T: Float> NoiseModule<Point2<T>> for OpenSimplex {
    ...
    fn get(&self, point: Point2<T>) -> T {
        ...
        value = value + gradient(&self.perm_table, xs_floor + one, ys_floor, dx1, dy1);

        // Contribution (0, 1)
        let dx2 = dx1 + one;
        let dy2 = dy1 - one;
        value = value + gradient(&self.perm_table, xs_floor, ys_floor + one, dx2, dy2);
        ...
        // Point (0, 0) or (1, 1)
        value = value + gradient(&self.perm_table, xs_floor, ys_floor, dx0, dy0);
...
impl<T: Float> NoiseModule<Point3<T>> for OpenSimplex {
    ...
    fn get(&self, point: Point3<T>) -> T {
        ...
        if frac_sum <= one {
            ...
            value = value + gradient(&self.perm_table, xsb, ysb, zsb, dx0, dy0, dz0);

            // Contribution at (1, 0, 0)
            let dx1 = dx0 - one - squish_constant;
            let dy1 = dy0 - squish_constant;
            let dz1 = dz0 - squish_constant;
            value = value + gradient(&self.perm_table, xsb + one, ysb, zsb, dx1, dy1, dz1);

            // Contribution at (0, 1, 0)
            let dx2 = dx0 - squish_constant;
            let dy2 = dy1 - one;
            let dz2 = dz1;
            value = value + gradient(&self.perm_table, xsb, ysb + one, zsb, dx2, dy2, dz2);

            // Contribution at (0, 0, 1)
            let dx3 = dx2;
            let dy3 = dy1;
            let dz3 = dz1 - one;
            value = value + gradient(&self.perm_table, xsb, ysb, zsb + one, dx3, dy3, dz3);
            ...
            let dy3 = dy0 - c0;
            let dz3 = dz0 - c0 + one;
            value = value + gradient(&self.perm_table, xsb + one, ysb + one, zsb, dx3, dy3, dz3);

            // Contribution at (1, 0, 1)
            let dx2 = dx3;
            let dy2 = dy3 + one;
            let dz2 = dz3 - one;
            value = value + gradient(&self.perm_table, xsb + one, ysb, zsb + one, dx2, dy2, dz2);

            // Contribution at (0, 1, 1)
            let dx1 = dx3 + one;
            let dy1 = dy3;
            let dz1 = dz2;
            value = value + gradient(&self.perm_table, xsb, ysb + one, zsb + one, dx1, dy1, dz1);

            // Contribution at (1, 1, 1)
            dx0 = dx3 - squish_constant;
            dy0 = dy3 - squish_constant;
            dz0 = dz2 - squish_constant;
            value = value + gradient(&self.perm_table, xsb + one, ysb + one, zsb + one, dx0, dy0, dz0);
        } else {
            ...
            value = value + gradient(&self.perm_table, xsb + one, ysb + one, zsb, dx4, dy4, dz4);

            // Contribution at (1, 0, 1)
            let dx5 = dx4;
            let dy5 = dy4 + one;
            let dz5 = dz4 - one;
            value = value + gradient(&self.perm_table, xsb + one, ysb, zsb + one, dx5, dy5, dz5);

            // Contribution at (0, 1, 1)
            let dx6 = dx4 + one;
            let dy6 = dy4;
            let dz6 = dz5;
            value = value + gradient(&self.perm_table, xsb, ysb + one, zsb + one, dx6, dy6, dz6);
        }
