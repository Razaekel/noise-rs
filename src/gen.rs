...
pub fn gradient_coherent_noise_3d<F: Float>(x: F, y: F, z: F, seed: int, quality: ::Quality) -> F {
    ...
    let z0 : int = z.floor().to_int().unwrap();
...
pub fn gradient_noise_3d<F:Float>(fx: F, fy: F, fz: F, ix: int, iy: int, iz: int, seed: int) -> F {

    // Generate an index into the random vector lookup-table
    ...
    let xv_point = fx - cast(ix).unwrap();
    let yv_point = fy - cast(iy).unwrap();
    let zv_point = fz - cast(iz).unwrap();

    // Compute the dot product and scale the value so the value ranges
    // from -1.0 to 1.0
    let v = ((xv_grad * xv_point)
    +(yv_grad * yv_point)
    +(zv_grad * zv_point)) / cast(SQRT_3).unwrap();
